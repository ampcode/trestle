...
test("coordination MCP validates sessions, persists evidence and attributes the transport", async () => {
  const ref = { provider: "codex", sessionId: "native-session" };
  const invoke = async (operation: string, args: Properties, requestId?: string): Promise<JsonValue> => {
    const result = await call("coordination", { operation, arguments: args, requestId });
    assert.equal(result.isError, false, result.text);
    return JSON.parse(result.text);
  };
  const rejected = await call("coordination", { operation: "registerSession", arguments: { ref } });
  assert.equal(rejected.isError, true);
  await invoke("registerSession", { ref }, "mcp-register");
  const created = expectObject(await invoke("createUnit", { id: "mcp-unit", title: "Migration", objective: "Separate module",
    acceptance: "Tests pass", scope: { graphRevision: 0, entityIds: [], sourceRevision: "pinned-source" }, lead: ref }, "mcp-create"));
  assert.deepEqual(created.lead, ref);
  const imported = expectArray(await invoke("indexArtifacts", { artifacts: [{ session: ref, nativeId: "verification",
    kind: "test-output", locator: { message: "verification" }, metadata: {}, text: "17 tests passed" }] }, "mcp-index"));
  const artifactId = expectObject(imported[0]).id;
  const bookmark = expectObject(await invoke("createBookmark", { unitId: "mcp-unit", artifactId, kind: "verification", description: "Test output" }, "mcp-bookmark"));
  const evidence = expectObject(await invoke("getBookmark", { id: bookmark.id }));
  assert.equal(expectObject(evidence.artifact).text, "17 tests passed");
  const search = expectObject(await invoke("searchArtifacts", { provider: "codex", query: "17 tests" }));
  assert.equal(expectObject(expectArray(search.items)[0]).id, artifactId);
  const history = expectObject(await invoke("getUnitHistory", { id: "mcp-unit" }));
  assert.equal(expectObject(expectArray(history.items)[0]).actor, "portal-service");
  const forged = await call("coordination", { operation: "registerSession", arguments: { ref, actor: "forged" }, requestId: "forged" });
