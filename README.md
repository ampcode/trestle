...
checkout with `npm ci && npm pack`, then install that `.tgz` in your application:

```sh
npm install --save-dev /path/to/trestle-0.1.0.tgz
npx trestle init --amp
npx trestle profile build
npx trestle extract
npx trestle resolve
npx trestle survey
```
...
An installed global `trestle` command defers to the project's local version.

`init` defaults to the current directory; `init ./other-repo` targets another
directory. It installs a local SDK/CLI dependency plus TypeScript tooling using
the project's declared package manager or lockfile (npm by default). Existing
scripts, dependencies, and module type are preserved. `--no-install` writes files
without running the package manager. Rerunning `init` preserves graph code and
...
├── trestle/
│   ├── package.json         # ESM scope; leaves the application's module type alone
│   ├── tsconfig.json        # npx tsc -p trestle/tsconfig.json
│   ├── profile.ts
...
- `.agents/skills/trestle-*`: profile, extraction, resolver, and loop guidance,
...
Outside an orb, run `npx trestle serve` directly.
...
Upgrade the local dependency with your package manager, then refresh Amp assets:

```sh
npm install --save-dev /path/to/new-trestle.tgz
npx trestle amp install
npx trestle profile check
npx tsc -p trestle/tsconfig.json
```
