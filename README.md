...
Install Trestle into the repository you want to analyze. Its CLI bootstraps a
graph project, its TypeScript SDK supports your vocabulary, extraction pipeline,
and resolvers, and its Amp integration teaches an agent how to build the graph.
You do not fork or copy the engine repository.
...
- **Designed for agents.** `trestle init --amp` installs the Amp plugin,
  authoring skills, project guidance, and orb bootstrap/service configuration.

## Requirements

- Node.js **≥ 23.6** (native TypeScript execution and `node:sqlite`)
- npm (for Trestle's isolated tooling, regardless of the application's language)
- Git

## Getting started

The package is not yet published. Build a local distribution in this contributor
checkout with `npm ci && npm pack`, then install that `.tgz` in the analyzed repository:

```sh
npm --prefix trestle install --workspaces=false --save-dev /path/to/trestle-0.1.0.tgz
npx --prefix trestle trestle init --amp
npm --prefix trestle run typecheck
npx --prefix trestle trestle profile build
npx --prefix trestle trestle extract
npx --prefix trestle trestle resolve
npx --prefix trestle trestle survey
```

After publication, the equivalent entrypoint is `npx trestle init --amp`.
An installed global `trestle` command defers to the version under `trestle/`.
For the `trestle` commands below, use that global command or prefix them with
`npx --prefix trestle` from the repository root.

`init` defaults to the current directory; `init ./other-repo` targets another
directory. It creates an independent npm environment under `trestle/` containing
the SDK/CLI, TypeScript ≥5.8, and Node types. The application need not use Node or
TypeScript: its root package files, lockfiles, compiler, and dependencies are
left untouched. `--no-install` writes files without running npm.
Rerunning `init` preserves graph code and
retries dependency installation; it does not reset your profile or resolvers.

```text
your-repository/
├── trestle.config.mts       # roots, paths, exclusions, visualization
├── trestle/
│   ├── package.json         # isolated SDK, compiler, Node types, and typecheck script
│   ├── package-lock.json    # commit this tooling lockfile
│   ├── node_modules/        # ignored; separate from application dependencies
│   ├── tsconfig.json        # npm --prefix trestle run typecheck
│   ├── profile.ts
│   ├── profile.lock.json   # generated by profile build; commit this
│   ├── extract/pipeline.ts
│   ├── resolvers/inventory.ts
│   └── .state/             # ignored database and projection
└── ...your application...
```

The root config's SDK import is type-only. The graph tsconfig maps it to the
isolated SDK's public declaration entrypoint, so checking the config needs no
root dependency. Native Node executes the graph code; this isolates npm packages
and TypeScript, not the Node runtime. Node ≥23.6 is still required on PATH for
bootstrap and services. Do not add `trestle/` to the application's workspaces;
the installer explicitly disables npm workspace handling to avoid hoisting.

The starter inventories repository files. Replace it with parsers and resolvers
for your migration questions. In Git repositories it lists tracked files and
untracked non-ignored files; it excludes dependencies, generated state, symlinks,
and configured `corpusExclude` file/directory paths. Outside Git it walks files
with explicit exclusions (there is no Git ignore evaluation). These filters
apply to `corpus.list()`, not to explicit pipeline reads or external tools.

For a separate migration workspace, initialize an empty directory and edit
`corpusRoots` to point to your estates, such as `["corpora"]`, then use
`trestle corpus add <url>`. Paths remain relative to the config file.

## How it works

Three authoring surfaces define a graph. Each is ordinary TypeScript importing
the public `trestle` SDK. The default paths below can be changed in the config.

**`trestle/profile.ts`** — the vocabulary: node kinds with identities, edge kinds
...
**`trestle/extract/pipeline.ts`** — transcription. It reads the corpus and emits
...
**`trestle/resolvers/*.ts`** — inference. Resolvers run in phase order, join facts
...
`trestle-extraction` skill includes a tool-selection index by language.

## Working with coding agents

`trestle init --amp` (or later `trestle amp install`) installs project-local:

- `.amp/plugins/trestle/`: the standalone plugin providing `trestle_auth`,
  `trestle_query`, `trestle_call`, and `trestle_amp` coordination tools.
- `.agents/skills/trestle-*`: profile, extraction, resolver, loop, and visualization guidance,
  including parser-selection references.
- A marked section in `AGENTS.md`, without replacing application guidance.
- A marked bootstrap section in `.agents/setup`, and a `trestle` service in
  `.amp/services.yaml`, preserving other services and setup code.

Reload plugins and skills in an active Amp thread, or start a new session.
The CLI writes integration files; it cannot reload an already-running agent.
Run `amp orb services ensure` to start the supervised service and get a portal.
Outside an orb, run `npx --prefix trestle trestle serve` directly.

`trestle amp install` refreshes owned assets after upgrades.
`trestle amp uninstall` removes only verified, unmodified owned assets and
sections; it leaves graph code, corpora, and databases intact. Both refuse
conflicting or user-modified managed files. Keep `.amp/trestle-install.json`
with installed files: it records ownership. Uninstall does not stop a running
service; use `amp orb service stop trestle` first when appropriate.
...
After upgrading, run `trestle project build` once to add relationship
...
Presentation lives in `trestle.config.mts` (or the legacy `.ts` config).
Add visualization settings to your existing config without replacing its paths:

```ts
import type { TrestleConfig } from "trestle";

export default {
  // Keep your existing corpus, profile, pipeline, resolver, and state paths.
  visualization: {
...
The installed `trestle-visualizing` skill guides agents through semantic kind
selection, readable node/edge pool budgets, label/color/size hierarchy, and
rendered verification. It distinguishes the full stored graph from the visible
canvas. Current configuration supports per-kind hiding/styling, not numeric
pool limits or a persisted initial layout. Above 2,000 visible nodes, the
bundled viewer can show a blank canvas; shrinking glyphs is not a fix. The
guidance explicitly calls out when a bounded-subgraph feature is needed rather
than claiming configuration alone can display an oversized inventory usefully.

## Corpora

By default, the current repository is the corpus. For additional, read-only
estates, configure a dedicated first `corpusRoots` entry such as `"corpora"`.
`corpus add` writes into that first root; it does not automatically choose
`corpora/` when the configured root is `"."`.

```sh
trestle corpus add <git-url> [name] [--ref <branch|tag|sha>]   # shallow submodule
trestle corpus add <archive-url> [name] [--sha256 <hash>]      # .tar.gz/.zip, manifest committed
trestle corpus restore                                     # refetch archive corpora
```

Git corpora are pinned by submodule SHA; archive corpora are pinned by a
committed `<corpus-root>/<name>.source.json` manifest. Neither commits corpus
bytes to your repository.
...
```sh
trestle coordination registerSession '{"ref":{"provider":"codex","sessionId":"native-session"}}' register-orders-1
trestle coordination createUnit '{"id":"orders","title":"Extract orders","objective":"Separate order processing","acceptance":"Contract tests pass","scope":{"graphRevision":0,"entityIds":["Module:orders"],"sourceRevision":"<source-commit>"},"lead":{"provider":"codex","sessionId":"native-session"}}' create-orders-1
trestle coordination listUnits '{}'
trestle coordination setUnitStatus '{"id":"orders","expectedRevision":1,"status":"active","reason":"Implementation started"}' activate-orders-1
```
...
Upgrade the isolated dependency, then refresh Amp assets:

```sh
npm --prefix trestle install --workspaces=false --save-dev /path/to/new-trestle.tgz
npx --prefix trestle trestle amp install
npx --prefix trestle trestle profile check
npm --prefix trestle run typecheck
```

Once published, use the desired registry version instead of the tarball.
Graph code is user-owned: upgrades do not overwrite it or move databases.
The public authoring API comes from `trestle`; coordination APIs remain available
from `trestle/coordination`. Engine internals are not public extension APIs.

Existing repo-style projects can keep `trestle.config.ts`, profile, pipeline,
resolver, and state paths unchanged. Move/copy those project files (including
state if you need history) into a project without the engine, install the
package, and keep the config. Do not retain `"name": "trestle"` and the old
engine `exports` in your application's package.json: they shadow the installed SDK.
Existing unmanaged Amp files must be reviewed and moved aside before installing
managed replacements. No automatic destructive migration is performed.

`init` refuses automatic migration of custom-layout projects without a
`trestle/package.json`; their existing CLI/configuration still works. For the
previous in-repository starter, install the SDK under `trestle/`, rerun `init`,
and add the `paths` mapping from the [current scaffold](./assets/project/trestle/tsconfig.json)
to its graph tsconfig.
Review any old root dependencies separately: initialization never removes them.

## Contributing

This repository contains the engine and packaged templates, not a root graph.
Run `npm ci`, then `npm run build` to build the JavaScript runtime, declarations,
standalone Amp adapter, and explorer assets. Run `npm run typecheck`, `npm test`,
and `npm run lint`. `npm run test:package` verifies a tarball installed outside the
checkout, including CommonJS application compatibility, the graph loop, SDK
typechecking, and the served explorer/MCP endpoint.
