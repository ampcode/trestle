...
1. **Start from the question, not the language.** Read `trestle.config.mts` (or legacy `trestle.config.ts`), then list the edge kinds in
   its configured profile (`trestle/profile.ts` by default) and what each needs from source. The minimum tier that
   answers each:
