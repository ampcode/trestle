---
name: trestle-profiles
description: Declares a Trestle profile — the node, edge, and fact vocabulary for a migration knowledge graph. Use when creating or editing trestle/profile.ts, choosing identities, adding kinds, or running trestle profile build/check.
---
...
- `trestle profile build` writes `trestle/profile.lock.json` by default (commit it; the hash is
